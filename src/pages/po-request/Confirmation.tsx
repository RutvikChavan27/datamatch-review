    ...
    <div className="container mx-auto px-4 max-w-3xl py-8">
      <div className="text-center mb-8 space-y-6">
        <div className="flex justify-center">
          <div className="rounded-full bg-po-statusApproved w-20 h-20 flex items-center justify-center">
            <div className="relative w-10 h-10">
              <Check className={`h-10 w-10 text-white animate-check-mark absolute left-0 top-0`} strokeWidth={3} />
            </div>
          </div>
        </div>
        
        <div className={`space-y-2 transition-opacity duration-300 ${animationComplete ? 'opacity-100' : 'opacity-0'}`}>
          <h1 className="text-3xl font-bold">Purchase Order Submitted!</h1>
          <p className="text-muted-foreground">
            Your purchase order has been successfully submitted and is now awaiting approval.
          </p>
        </div>
      </div>
      
      <Card className={`mb-8 border-l-4 border-l-po-statusSubmitted transition-opacity duration-500 ${animationComplete ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="font-medium">{po.createdAt?.toLocaleDateString()}</p>
            </div>
          </div>
          ...
          <div>
            <p className="text-sm text-muted-foreground mb-2">Approval Timeline</p>
          ...
          <div className="bg-muted/30 p-4 rounded-md text-sm">
            <p>
              <span className="font-medium">Expected approval timeline:</span> Your purchase order is expected to be processed within 2-3 business days.
            </p>
          </div>
        </CardContent>
      </Card>
      
      <div className={`flex gap-4 justify-center transition-opacity duration-700 ${animationComplete ? 'opacity-100' : 'opacity-0'}`}>
        <Button 
          variant="outline" 
        ...
        <Button 
          ...
          className="flex-1 sm:flex-none bg-po-brand hover:bg-po-hover"
        >
          View My POs
        </Button>
      </div>
