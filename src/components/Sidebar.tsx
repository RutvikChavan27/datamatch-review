...
const Sidebar: React.FC<SidebarProps> = ({ enabledModules }) => {
  ...
  const isActive = (path?: string) => {
    ...
    if (path === '/documents/invoices' && location.pathname.startsWith('/documents/invoices/')) return true;
    if (path === '/documents/purchase-orders' && location.pathname.startsWith('/documents/purchase-orders/')) return true;
    if (path === '/documents/goods-receipt-notes' && location.pathname.startsWith('/documents/goods-receipt-notes/')) return true;
    if (path === '/po-requests' && location.pathname.startsWith('/po-requests')) return true;
    if (path === '/matching' && (location.pathname.startsWith('/matching/') || location.pathname === '/matching')) return true;
    if (path === '/workflows' && (location.pathname.startsWith('/workflows/') || location.pathname === '/workflows')) return true;
    return false;
  };
  ...
  const navItems: NavCategory[] = [
    {
      ...
      items: [
        ...
        { name: 'Workflows', path: '/workflows', always: true, icon: Workflow },
        { name: 'PO Requests', path: '/po-requests', condition: enabledModules.poRequest, hasNotification: hasNewRequests, icon: FileText },
        { name: 'Document Matching', path: '/matching', always: true, icon: GitMerge },
      ...
      return (
        <div key={item.name} className="mb-1">
          ...
          <ul className="ml-2 pl-3 border-l border-gray-100 space-y-1">
            {item.subItems
              .filter(sub => sub.always || sub.condition === undefined || sub.condition)
              .map(sub => (
                <li key={sub.name}>
                  <Link
                    to={sub.path || '#'}
  ...
  const handleExpandSection = (section: 'PRODUCTIVITY ENGINE' | 'DOCUMENT MATCHING') => {
          ...
          <DropdownMenu>
            <DropdownMenuTrigger className="w-full flex items-center space-x-3 p-2 rounded-md hover:bg-gray-50 transition-colors">
              <Avatar className="h-8 w-8">
                ...
                <AvatarFallback className="bg-blue-100 text-blue-700 text-sm font-semibold">JS</AvatarFallback>
              </Avatar>
