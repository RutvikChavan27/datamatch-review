...
import { Eye, MessageSquare, MoreHorizontal, User, Calendar, ChevronUp, ChevronDown } from 'lucide-react';
...
const POTable: React.FC<POTableProps> = ({ purchaseOrders, itemsPerPage = 15 }) => {
  ...
  const hasOngoingDiscussion = (po: PurchaseOrder) => {
    return po.discussion && po.discussion.isActive && po.discussion.messages.length > 0;
  };
  ...
  const getPageNumbers = () => {
    ...
    if (totalPages <= maxPagesToShow) {
      ...
      if (currentPage <= 3) {
        ...
        // At the end, show 1 ... n-3 n-2 n-1 n
        pages.push(1);
        pages.push('ellipsis');
  ...
  // Debug log to see if we have any POs with discussion status
  console.log("POs with discussion status:", purchaseOrders.filter(po => po.status === 'discussion'));
                  ...
                  >
                    <TableCell className="font-mono text-sm font-medium py-2 border-r-0 text-foreground truncate w-[100px]">
                      <div className="flex items-center gap-2">
                        <div className="truncate" title={po.reference}>{po.reference}</div>
                        {(hasUnreadMessages(po) || hasOngoingDiscussion(po)) && (
                    ...
                    <TableCell className="py-2 border-r-0 text-sm text-foreground truncate w-[180px]" title={po.title}>
                      {po.title}
                    </TableCell>
                    <TableCell className="py-2 border-r-0 w-[120px]">
                      <StatusBadge status={po.status} />
                    </TableCell>
                    <TableCell className="py-2 border-r-0 text-sm text-foreground truncate w-[160px]" title={po.vendor}>
                      {po.vendor}
                    </TableCell>
                    <TableCell className="py-2 border-r-0 w-[100px]">
                      <Badge variant="secondary" className="bg-gray-100 text-gray-700 hover:bg-gray-200" title={po.department}>{po.department}</Badge>
                    </TableCell>
                    <TableCell className="py-2 border-r-0 text-sm text-foreground truncate w-[150px]">
                      <div className="flex items-center gap-1">
                        <User className="h-3 w-3 text-gray-500" />
                        <div className="truncate" title="Alex Johnson">Alex Johnson</div>
                      </div>
                    </TableCell>
                    <TableCell className="py-2 border-r-0 text-sm text-foreground w-[120px]">
                      <div className="flex items-center gap-1">
                        <Calendar className="h-3 w-3 text-gray-500" />
                        <div className="truncate" title={po.createdAt.toLocaleDateString()}>{po.createdAt.toLocaleDateString()}</div>
                      </div>
                    </TableCell>
                    <TableCell className="py-2 border-r-0 text-sm text-foreground w-[80px]">
                      <div className="truncate" title={`${po.lineItems.length} items`}>{po.lineItems.length} items</div>
                    </TableCell>
                    <TableCell className="font-medium py-2 border-r-0 text-sm text-foreground w-[120px] text-right">
                      <div className="truncate" title={formatCurrency(po.totalAmount)}>{formatCurrency(po.totalAmount)}</div>
                    </TableCell>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <TooltipProvider>
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button variant="ghost" onClick={(e) => handleQuickView(e, po)}>
