...
const MatchingQueue = () => {
  ...
  const getIssuesIndicator = (
    ...
    if (statusType === "processing" || statusType === "failed") {
      return (
        <span className="text-muted-foreground text-sm font-roboto">—</span>
      );
    }
    ...
    return (
      <div className="flex items-center space-x-1">
        <span className="text-foreground text-sm font-medium font-inter">
          {totalIssues}
        </span>
  ...
  const getDaysDisplay = (days: number) => {
    if (days > 10) {
      return (
        <span className="text-orange-600 font-bold text-sm font-inter">
          {days}
        </span>
      );
    } else if (days > 7) {
      ...
      return (
        <span className="text-muted-foreground text-sm font-roboto">
          {days}
        </span>
      );
    }
    ...
    if (status === "Ready for Verification") {
      return (
        <Button className="button-primary" asChild>
          <Link to={`/matching/sets/${setId}`}>Verify</Link>
        </Button>
      );
    }
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button variant="ghost" asChild>
              <Link
                to={`/matching/sets/${setId}?status=${encodeURIComponent(
                  status
                )}`}
              >
  ...
  const filteredSets = documentSets
    .filter((set) => {
      ...
      if (
        ...
        set.statusType !== "auto-approved" &&
        set.statusType !== "approved"
      )
        return false;
      if (
        statusFilter !== "all" &&
        statusFilter !== "verified" &&
        set.statusType !== statusFilter
      )
        return false;
      ...
      if (
        searchTerm &&
        !set.vendor.toLowerCase().includes(searchTerm.toLowerCase()) &&
        !set.poNumber.toLowerCase().includes(searchTerm.toLowerCase())
      )
        return false;

      // Smart filter combinations
      if (smartFilter === "urgent")
        return set.daysInQueue > 7 || set.priority === "high";
      if (smartFilter === "high-value") return set.totalAmount > 10000;
      if (smartFilter === "has-issues")
        return set.majorIssues > 0 || set.minorIssues > 0;
      if (smartFilter === "missing-docs")
        return !set.documents.po || !set.documents.grn;
    ...
    .sort((a, b) => {
      ...
      switch (sortBy) {
        case "priority":
          const aPriority =
            a.daysInQueue > 10 || a.priority === "high"
              ? 3
              : a.daysInQueue > 7 || a.priority === "medium"
              ? 2
              : 1;
          const bPriority =
            b.daysInQueue > 10 || b.priority === "high"
              ? 3
              : b.daysInQueue > 7 || b.priority === "medium"
              ? 2
              : 1;
  ...
  const getStatusCounts = () => {
    const counts = {
      ...
      incomplete: documentSets.filter((s) => s.statusType === "incomplete")
        .length,
      "ready-for-review": documentSets.filter(
        (s) => s.statusType === "ready-for-review"
      ).length,
      verified: documentSets.filter(
        (s) => s.statusType === "auto-approved" || s.statusType === "approved"
      ).length,
  ...
  const getCountBadge = (tabKey: string, count: number, isActive: boolean) => {
    if (isActive) {
      ...
      switch (tabKey) {
        ...
        case "incomplete":
          // Missing Documents - use red colors like rejected
          return (
        ...
        case "ready-for-review":
          // Ready for Verification - use yellow/pending colors
          return (
        ...
        case "verified":
          // Verified - use green colors like approved
          return (
    ...
    <div className="space-y-2 px-4 pt-4 pb-2 max-w-full overflow-x-hidden">
      ...
      <div className="flex items-center justify-between py-1">
        ...
        <div className="flex space-x-2 items-center">
          ...
          <div className="flex items-center space-x-2 text-sm text-muted-foreground font-roboto"></div>
        </div>
      </div>
        ...
        <div className="flex items-center gap-2 bg-gray-100 p-1 rounded-full">
          {[
            ...
            },
            {
              ...
              count: statusCounts["ready-for-review"],
            },
            {
              key: "verified",
              label: "Verified",
      ...
      >
        ...
        <TabsContent
          value={statusFilter}
          className="mt-2 shadow-lg shadow-black/5"
        >
          <Card className="overflow-hidden">
            ...
            <div
              ...
              style={{
                ...
                height:
                  paginatedSets.length > 15 ? `calc(100vh - 320px)` : "auto",
              }}
            >
              <Table className="min-w-full" style={{ tableLayout: "fixed" }}>
                <TableHeader className="sticky top-0 z-10">
                  <TableRow className="bg-muted/50 border-b border-border hover:bg-muted/50">
                    <TableHead
                      className="w-12 font-semibold border-r-0 text-sm h-12 border-b border-t"
                      style={{
                        backgroundColor: "#DFE7F3",
                        borderBottomColor: "#c9d1e0",
                        borderTopColor: "#c9d1e0",
                    ...
                    >
                      <Checkbox
                        checked={
                          selectedItems.size === paginatedSets.length &&
                    ...
                    <SortableHeader field="vendor" className="w-[160px]">
                      Vendor Name
                    </SortableHeader>
                    <TableHead
                      className="font-semibold w-[120px] border-r-0 text-sm text-foreground h-12 border-b border-t"
                      style={{
                        backgroundColor: "#DFE7F3",
                        borderBottomColor: "#c9d1e0",
                        borderTopColor: "#c9d1e0",
                    ...
                    <TableHead
                      className="font-semibold w-[140px] border-r-0 text-sm text-foreground h-12 border-b border-t"
                      style={{
                        backgroundColor: "#DFE7F3",
                        borderBottomColor: "#c9d1e0",
                        borderTopColor: "#c9d1e0",
                    ...
                    <SortableHeader field="issues" className="w-[80px]">
                      Issues
                    </SortableHeader>
                    <SortableHeader field="amount" className="w-[120px]">
                      Amount
                    </SortableHeader>
                    <SortableHeader field="age" className="w-[80px]">
                      Days
                    </SortableHeader>
                    <TableHead
                      className="font-semibold w-[140px] border-r-0 text-sm text-foreground h-12 border-b border-t"
                  ...
                  {paginatedSets.map((set) => (
                    ...
                    >
                      ...
                      <TableCell className="py-2 border-r-0 w-[140px]">
                        {getStatusBadge(set.status, set.statusType)}
                      </TableCell>
                      ...
                      <TableCell className="font-medium py-2 border-r-0 text-sm text-foreground w-[120px]">
                        {formatCurrency(set.totalAmount)}
                      </TableCell>
                      <TableCell className="py-2 border-r-0 text-sm text-muted-foreground w-[80px]">
                        {getDaysDisplay(set.daysInQueue)}
                      </TableCell>
                      <TableCell className="py-2 border-r-0 w-[140px]">
