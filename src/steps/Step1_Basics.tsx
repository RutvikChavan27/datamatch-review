...
import { Workflow, FileText, Target, AlertCircle, CheckCircle, ChevronLeft } from 'lucide-react';
...
export const Step1_Basics: React.FC<Step1Props> = ({
  ...
  const [selectedTemplate, setSelectedTemplate] = React.useState<typeof workflowTemplates[0] | null>(null);
  ...
  const workflowTemplates = [{
    ...
    description: "Route new documents to appropriate reviewers based on content type and priority",
    trigger: {
      type: 'document_uploaded',
      category: 'Document Management',
      config: {
    ...
    description: "Automatically categorize and assign support tickets to the right team members",
    trigger: {
      type: 'form_submitted',
    ...
    description: "Streamline new hire paperwork processing and task assignments",
    trigger: {
      type: 'form_submitted',
      category: 'Forms & Submissions',
      config: {
    ...
    actions: [{
      ...
      type: 'create_tasks',
      config: {
        tasks: ['setup_email_account', 'create_id_badge', 'schedule_orientation', 'assign_equipment'],
        assign_to: 'hr_team'
      },
      order: 0
    }, {
  ...
  const isNextEnabled = () => {
    return selectedTemplate !== null || (watchedValues.name?.trim().length > 0 && isValid);
  };
  return <div className="space-y-6">
      ...
      <div className="pt-6 mb-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h2 className="text-lg font-bold text-foreground font-inter">Create Your Workflow</h2>
        </div>
        <p className="text-muted-foreground font-roboto">Choose a pre-built template or start from scratch</p>
      </div>
      ...
      <div className="bg-card rounded-xl border p-6 shadow-md">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground font-roboto">
              {isTemplateMode ? 'Update the workflow name and description as needed.' : 'Create a custom workflow by configuring each step manually.'}
            </p>
          </div>
        </div>
        ...
        <form className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="workflow-name" className="text-sm font-medium text-foreground font-roboto">
              Workflow Name 
              ...
              <Input
                ...
                className={`
                  ${errors.name && showValidation
                    ...
                    : !errors.name && watchedValues.name && watchedValues.name.length > 0
          ...
          <div className="space-y-2">
            <Label htmlFor="workflow-description" className="text-sm font-medium text-foreground font-roboto">
              Description
            </Label>
            <div className="relative">
              <textarea
                ...
                className={`
                  ...
                  ${errors.description && showValidation
                    ? 'border-red-500 bg-red-50 focus-visible:ring-red-500' 
                    : !errors.description && watchedValues.description && watchedValues.description.length > 0
                    ? 'border-green-500 bg-green-50 focus-visible:ring-green-500'
                    : ''
                  }
                `}
      ...
      {!isTemplateMode && <>
          <div className="flex items-center justify-center my-8">
            ...
            <span className="px-4 text-sm font-medium text-muted-foreground bg-background font-roboto">Or Select Pre-built Template</span>
            <div className="flex-1 h-px bg-border"></div>
          </div>
        ...
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            <h3 className="text-base font-semibold text-foreground font-roboto">Pre-built Ready to Use Workflow Templates</h3>
            <p className="text-sm text-muted-foreground font-roboto">
              Complete workflows with triggers and actions already configured. Select one to customize or use as-is.
            </p>
          </div>
        </div>
        
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {workflowTemplates.map((template, index) => <TooltipProvider key={index}>
              <Tooltip>
                <TooltipTrigger asChild>
                   <div onClick={() => handleTemplateSelect(template)} className={`group p-3 border-2 rounded-xl cursor-pointer transition-all duration-200 hover:shadow-lg ${
               ...
               <div className="space-y-1">
                 <div className="flex items-center gap-2 text-xs">
                   ...
                   <span className="font-medium text-foreground">{template.trigger.category}</span>
                 </div>
                 <div className="flex items-center gap-2 text-xs">
                   <div className="w-2 h-2 rounded-full bg-green-500"></div>
                   <span className="text-muted-foreground">Actions:</span>
                   <span className="font-medium text-foreground">{template.actions.length} configured</span>
                 </div>
               </div>
